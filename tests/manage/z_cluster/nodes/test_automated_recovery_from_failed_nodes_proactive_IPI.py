...
class TestAutomatedRecoveryFromFailedNodes(ManageTest):
    ...
    def test_automated_recovery_from_failed_nodes_IPI_proactive(
        ...
        machine_name = machine.get_machine_from_node_name(osd_running_nodes[0])
        log.info(f"{osd_running_nodes[0]} associated machine is {machine_name}")
        ...
        log.info(
            f"{osd_running_nodes[0]} associated machineset is {machineset_name}"
        )
